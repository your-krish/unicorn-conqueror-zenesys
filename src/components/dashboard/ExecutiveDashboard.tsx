...
import { 
  ...
  MapPin, Activity, Network, Sparkles, Lightbulb, AlertOctagon,
  RefreshCw, BarChart2
} from 'lucide-react';
...
import { AiBusinessInsightsView } from '../ai/AiBusinessInsightsView';
import { AiPredictiveAnalyticsView } from '../ai/AiPredictiveAnalyticsView';
import { AiAnomalyDetectionView } from '../ai/AiAnomalyDetectionView';
...
export const ExecutiveDashboard: React.FC<ExecutiveDashboardProps> = ({
  ...
  const { analysis, loading: aiLoading, refreshAnalysis } = useAiAnalytics();
  const [activeAiTab, setActiveAiTab] = useState<'insights' | 'predictions' | 'anomalies'>('insights');
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-2 border-b border-[var(--border-hairline)]">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-4 py-2 rounded-2xl bg-emerald-500 hover:bg-emerald-400 text-neutral-950 text-xs font-bold transition-all shadow-md shadow-emerald-500/20 cursor-pointer"
          >
      ...
      {/* AI OPERATIONS INTELLIGENCE SYNTHESIS WIDGET */}
      <div className="p-4 rounded-3xl bg-[var(--bg-surface-elevated)] border border-emerald-500/30 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="space-y-0.5">
            <div className="flex items-center gap-2">
              <span className="text-[10px] font-mono uppercase font-bold text-emerald-600 dark:text-emerald-400 bg-emerald-500/10 px-2 py-0.5 rounded-full border border-emerald-500/20">
                Gemini AI Intelligence Matrix
              </span>
              <span className="text-[11px] font-mono text-[var(--text-metadata)]">
                {analysis?.anomalies.length || 4} Anomalies • {analysis?.predictions.length || 4} Forecasts Active
              </span>
            </div>
            <p className="text-xs text-[var(--text-primary)] font-medium max-w-3xl leading-relaxed">
              {analysis?.executive_summary || 'Evaluating enterprise logistics, Pune semiconductor depletion, and revenue exposure mitigation vectors.'}
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          <button
            ...
            disabled={aiLoading}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-[var(--bg-surface)] hover:bg-[var(--bg-surface-elevated)] border border-[var(--border-subtle)] text-xs font-mono text-[var(--text-secondary)] transition-all cursor-pointer"
          >
          ...
          <button
            onClick={() => onNavigateTab('ai-analytics')}
            className="flex items-center gap-1.5 px-4 py-2 rounded-2xl bg-emerald-500 hover:bg-emerald-400 text-neutral-950 font-bold text-xs transition-all shadow-md shadow-emerald-500/20 cursor-pointer"
          >
      ...
      {/* EMBEDDED AI OPERATIONS INTELLIGENCE MATRIX (Insights, Predictive Analytics & Anomaly Detection) */}
      <div className="p-6 rounded-3xl bg-[var(--bg-surface)] border border-[var(--border-subtle)] space-y-6 shadow-sm">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-[var(--border-subtle)]">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h3 className="text-base font-bold text-[var(--text-primary)]">
                  AI Operations Intelligence & Horizon Modeling
                </h3>
                <span className="text-[10px] font-mono uppercase font-bold text-emerald-600 dark:text-emerald-400 bg-emerald-500/10 px-2 py-0.5 rounded-full border border-emerald-500/20">
                  Live Gemini Engine
                </span>
              ...
              <p className="text-xs text-[var(--text-metadata)]">
                Autonomous multi-vector analysis across business insights, regression forecasts, and statistical variance anomalies.
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={() => refreshAnalysis()}
              disabled={aiLoading}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-[var(--bg-surface-elevated)] hover:bg-[var(--bg-surface)] border border-[var(--border-subtle)] text-xs font-mono text-[var(--text-secondary)] transition-all cursor-pointer"
            >
              <RefreshCw className={`h-3 w-3 ${aiLoading ? 'animate-spin' : ''}`} />
              <span>{aiLoading ? 'Synthesizing...' : 'Refresh AI Analysis'}</span>
            </button>
            <button
              onClick={() => onNavigateTab('ai-analytics')}
              className="text-xs font-mono font-bold text-emerald-600 dark:text-emerald-400 hover:underline flex items-center gap-1 cursor-pointer ml-2"
            >
              <span>Full Screen Suite</span>
              <ArrowRight className="h-3 w-3" />
            </button>
          </div>
        </div>

        {/* 3 AI Sub-navigation buttons */}
        <div className="flex flex-wrap items-center gap-2">
          <button
          ...
          >
            ...
            {analysis && (
              <span className={`text-[10px] font-mono px-1.5 py-0.2 rounded-full ${
                activeAiTab === 'insights' ? 'bg-neutral-950 text-emerald-400' : 'bg-emerald-500/15 text-emerald-600 dark:text-emerald-400'
              }`}>
          ...
          >
            ...
            {analysis && (
              <span className={`text-[10px] font-mono px-1.5 py-0.2 rounded-full ${
                activeAiTab === 'predictions' ? 'bg-neutral-950 text-emerald-400' : 'bg-cyan-500/15 text-cyan-600 dark:text-cyan-400'
              }`}>
          ...
          <button
            onClick={() => setActiveAiTab('anomalies')}
            className={`flex items-center gap-2 px-4 py-2 rounded-2xl text-xs font-bold transition-all cursor-pointer ${
          ...
          >
            ...
            {analysis && (
              <span className={`text-[10px] font-mono px-1.5 py-0.2 rounded-full ${
                activeAiTab === 'anomalies' ? 'bg-neutral-950 text-emerald-400' : 'bg-rose-500/15 text-rose-600 dark:text-rose-400'
              }`}>
        ...
        <div className="pt-2">
          {activeAiTab === 'insights' && (
            <AiBusinessInsightsView compact={true} />
          )}
