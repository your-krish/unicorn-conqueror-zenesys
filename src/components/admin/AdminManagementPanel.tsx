...
export const AdminManagementPanel: React.FC = () => {
  ...
  const handleTestSupabase = async () => {
    ...
    try {
      ...
      setTestResult(res);
      if (res.connected) {
        showNotification('Supabase backend connection verified successfully!');
      }
    } catch (err: any) {
  ...
  const handleSaveSupabaseCredentials = (e: React.FormEvent) => {
    ...
    setSupabaseConfig(getActiveSupabaseConfig());
    showNotification('Supabase connection details updated successfully.');
    handleTestSupabase();
  };
  ...
  return (
    <div id="admin-management-panel" className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 border-b border-neutral-800 pb-3">
        <div className="flex items-center gap-1.5 overflow-x-auto pb-1 sm:pb-0">
          {[
            ...
            { id: 'supabase_sync', label: 'Supabase Cloud Backend', icon: Database, count: 'LIVE' },
          ].map(tab => {
            const Icon = tab.icon;
      ...
      {/* 6. SUPABASE CLOUD BACKEND STATUS & SCHEMA */}
      {activeTab === 'supabase_sync' && (
        <div className="space-y-6">
          <div className="p-6 bg-neutral-900/60 border border-neutral-800 rounded-xl space-y-6">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-neutral-800 pb-5">
              <div className="flex items-start gap-3.5">
                ...
                <div>
                  <div className="flex items-center gap-2.5">
                    <h2 className="text-base font-bold text-white">Live Supabase Backend Integration</h2>
                    <span className="text-[10px] font-mono uppercase px-2 py-0.5 rounded-full bg-emerald-500/15 text-emerald-400 border border-emerald-500/30 font-bold flex items-center gap-1">
                      <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse"></span>
                      CONNECTED: {supabaseConfig.projectId}
                    </span>
                  </div>
                  <p className="text-xs text-neutral-400 mt-1 max-w-2xl">
                    Every incident created, updated, or deleted in this panel or via operational triggers is automatically saved directly to your Supabase backend table <code className="text-amber-400 font-mono text-[11px] bg-neutral-950 px-1 py-0.5 rounded border border-neutral-800">public.incidents</code> in real-time.
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 shrink-0">
                <button
                  ...
                  className="flex items-center gap-1.5 px-3.5 py-2 bg-neutral-800 hover:bg-neutral-700 text-neutral-200 text-xs font-semibold rounded-lg border border-neutral-700 transition-all cursor-pointer"
                >
                  <RefreshCw className={`w-3.5 h-3.5 ${testingSupabase ? 'animate-spin text-amber-400' : ''}`} />
                  <span>{testingSupabase ? 'Testing...' : 'Test Connection'}</span>
                </button>
                <a
                  href={`https://supabase.com/dashboard/project/${supabaseConfig.projectId}/editor`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center gap-1.5 px-3.5 py-2 bg-emerald-500 hover:bg-emerald-400 text-neutral-950 text-xs font-bold rounded-lg transition-all shadow-md shadow-emerald-500/20"
                >
            ...
            {testResult && (
              <div className={`p-4 rounded-xl border flex items-start gap-3 text-xs ${
                ...
                <div>
                  <div className="font-semibold">{testResult.connected ? 'Connection Succeeded' : 'Connection Warning'}</div>
            ...
            <form onSubmit={handleSaveSupabaseCredentials} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <span className="text-[11px] text-neutral-500 mt-1 block">
                    Target Project ID: <code className="text-amber-400 font-mono">{supabaseConfig.projectId}</code>
                  </span>
                </div>
                ...
                <div>
                  ...
                  />
                  <span className="text-[11px] text-neutral-500 mt-1 block">
                    Publishable client key with realtime and table read/write policies
                  </span>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between pt-2">
                <div className="text-[11px] text-neutral-500 font-mono">
                  Auto-sync active on incident CRUD operations
                </div>
                <button
                  type="submit"
                  className="px-4 py-2 bg-amber-400 hover:bg-amber-300 text-neutral-950 text-xs font-bold rounded-lg transition-colors flex items-center gap-1.5 cursor-pointer"
                >
          ...
          {/* SQL Schema helper in case the table needs to be created in Supabase SQL editor */}
          <div className="p-6 bg-neutral-900/40 border border-neutral-800 rounded-xl space-y-3">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-xs font-bold text-white uppercase tracking-wider flex items-center gap-2">
                  <span>Supabase SQL Table Definition</span>
                  <span className="text-[10px] text-neutral-400 font-mono lowercase">(public.incidents)</span>
                </h3>
                <p className="text-[11px] text-neutral-400 mt-0.5">
                  If you haven't created the <code className="text-amber-400 font-mono">incidents</code> table in your Supabase project yet, run this SQL script in your Supabase SQL Editor:
                </p>
              </div>
              <button
                type="button"
                onClick={handleCopySchema}
                className="flex items-center gap-1.5 px-3 py-1.5 bg-neutral-800 hover:bg-neutral-700 text-neutral-200 text-xs font-medium rounded-lg border border-neutral-700 transition-colors cursor-pointer"
              >
                {copiedSchema ? <Check className="w-3.5 h-3.5 text-emerald-400" /> : <Layers className="w-3.5 h-3.5" />}
                <span>{copiedSchema ? 'Copied!' : 'Copy SQL Script'}</span>
              </button>
            </div>

            <pre className="p-4 bg-neutral-950 border border-neutral-800/80 rounded-lg text-emerald-400 font-mono text-[11px] overflow-x-auto leading-relaxed max-h-56">
              {SUPABASE_SQL_SCHEMA}
            </pre>
          </div>
        </div>
